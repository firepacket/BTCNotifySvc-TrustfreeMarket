...
namespace BitCoinSharp
{
    ...
    public class Transaction : Message
    {
        ...
        public IEnumerable<Tuple<string, ulong>> GetOutputBalances()
        {
            List<Tuple<string, ulong>> list = new List<Tuple<string, ulong>>();
            foreach (var @out in _outputs)
            {
                string toAddr = null;
                try
                {
                    toAddr = new Address(Params, @out.ScriptPubKey.PubKeyHash).ToString();
                }
                catch (Exception e)
                { }
                ...
                if (string.IsNullOrEmpty(toAddr))
                    ...
                    yield return new Tuple<string, ulong>(toAddr, @out.Value);
            }
        }

        public IEnumerable<string> GetInputAddresses()
        {
            foreach (var @in in _inputs)
            {
                string addr = null;
                try
                {
                    addr = @in.ScriptSig.FromAddress.ToString();
                }
                catch (Exception e)
                { }
