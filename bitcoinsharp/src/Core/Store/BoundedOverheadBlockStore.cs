...
namespace BitCoinSharp.Store
{
    ...
    public class BoundedOverheadBlockStore : IBlockStore
    {
        ...
        private class Record
        {
            ...
            {
                if (!channel.CanRead)
                    channel = vanishingfile.Open(FileMode.Open, FileAccess.ReadWrite);
        ...
        static FileInfo vanishingfile = new FileInfo("custom.blockchain");
        ...
        public BoundedOverheadBlockStore(NetworkParameters @params, FileInfo file)
        {
            ...
            {
                Console.WriteLine("ERROR: " + "failed to load block store from file", e);
        ...
        {
            Console.WriteLine(string.Format("Reading block store from {0}", file));
            if (_channel != null)
            {
                _channel.Dispose();
            }
            ...
            try
            {
                ...
                Console.WriteLine(string.Format("Read chain head from disk: {0}", _chainHead));
        ...
        public void Put(StoredBlock block)
        {
            lock (this)
            {
                ...
                {
                    if (_channel == null)
                        _channel = vanishingfile.Open(FileMode.Open, FileAccess.ReadWrite);
        ...
        {
            if (_buf == null)
                _buf = ByteBuffer.Allocate(Record.Size);
