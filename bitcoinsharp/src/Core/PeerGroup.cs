    ...
    public class PeerGroup
    {
        ...
        public bool BroadcastTransaction(Transaction tx)
        {
            ...
            lock (_peers)
            {
                foreach (var peer in _peers)
                {
                    ...
                    {
                        Console.WriteLine("ERROR: " + "failed to broadcast to " + peer, e);
                    }
                }
            }
            return success;
        ...
        private void DiscoverPeers()
        {
            foreach (var peerDiscovery in _peerDiscoverers)
            {
                ...
                catch (PeerDiscoveryException e)
                {
                    ...
                    Console.WriteLine("ERROR: " + "Failed to discover peer addresses from discovery source", e);
                    return;
                }
        ...
        private void TryNextPeer()
        {
            ...
            while (true)
            {
                            ...
                            catch (PeerException ex)
                            {
                                ...
                                if (cause is SocketException)
                                {
                                    if (((SocketException) cause).SocketErrorCode == SocketError.TimedOut)
                                        Console.WriteLine("Timeout talking to " + peer + ": " + cause.Message);
                                    else
                                        Console.WriteLine("Could not connect to " + peer + ": " + cause.Message);
                                }
                                else if (cause is IOException)
                                {
                                    Console.WriteLine("Error talking to " + peer + ": " + cause.Message);
                                }
                                else
                                {
                                    Console.WriteLine("ERROR: " + "Unexpected exception whilst talking to " + peer, ex);
                                }
                            }
                            finally
                            {
                ...
                {
                    // Fatal error
                    Console.WriteLine("ERROR: " + "Block store corrupt?", e);
                    _running = false;
        ...
        private void StartBlockChainDownloadFromPeer(Peer peer)
        {
            lock (this)
            {
                ...
                {
                    Console.WriteLine("ERROR: " + "failed to start block chain download from " + peer, e);
                    return;
                }
                _downloadPeer = peer;
            }
