    ...
    public class Wallet
    {
        ...
        private void Receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType, bool reorg)
        {
            lock (this)
            {
                ...
                if (!reorg)
                {
                    Console.WriteLine(string.Format("Received tx{0} for {1} BTC: {2}", sideChain ? " on a side chain" : "",
                ...
                if (Pending.TryGetValue(txHash, out wtx))
                {
                    ...
                    else if (sideChain)
                    {
                        ...
                        if (_inactive.ContainsKey(wtx.Hash))
                            Console.WriteLine("Saw a transaction be incorporated into multiple independent side chains");
                ...
                Console.WriteLine(string.Format("Balance is now: {0}", Utils.BitcoinValueToFriendlyString(GetBalance())));
        ...
        private void ProcessTxFromBestChain(Transaction tx)
        {
            ...
            if (!tx.GetValueSentToMe(this).Equals(0))
            {
                ...
                Console.WriteLine("  new tx ->unspent");
            ...
            else
            {
                ...
                Console.WriteLine("  new tx ->spent");
        ...
        private void UpdateForSpends(Transaction tx)
        {
            ...
            foreach (var input in tx.Inputs)
            {
                ...
                if (result == TransactionInput.ConnectionResult.AlreadySpent)
                {
                    ...
                    {
                        Console.WriteLine(string.Format("Saw double spend from chain override pending tx {0}", connected.HashAsString));
                        Console.WriteLine("  <-pending ->dead");
        ...
        private void MaybeMoveTxToSpent(Transaction tx, String context)
        {
            if (tx.IsEveryOutputSpent())
            {
                ...
                if (Unspent.Remove(tx.Hash))
                {
                    ...
                    {
                        Console.WriteLine("  " + context + " <-unspent");
                        Console.WriteLine("  " + context + " ->spent");
                    }
                    Spent[tx.Hash] = tx;
                }
            }
        ...
        internal void ConfirmSend(Transaction tx)
        {
            lock (this)
            {
                ...
                Console.WriteLine(string.Format("confirmSend of {0}"), tx.HashAsString);
        ...
        internal Transaction CreateSend(Address address, ulong nanocoins, Address changeAddress)
        {
            lock (this)
            {
                Console.WriteLine("Creating send tx to " + address + " for " +
                ...
                {
                    Console.WriteLine("Insufficient value in wallet for send, missing " +
                ...
                if (change > 0)
                {
                    ...
                    // back to us.
                    Console.WriteLine("  with " + Utils.BitcoinValueToFriendlyString((ulong) change) + " coins change");
                ...
                Console.WriteLine(string.Format("  created {0}", sendTx.HashAsString));
                return sendTx;
            }
        }
        ...
        internal void Reorganize(IList<StoredBlock> oldBlocks, IList<StoredBlock> newBlocks)
        {
            lock (this)
            {
                ...
                Console.WriteLine("  Old part of chain (top to bottom):");
                foreach (var b in oldBlocks) Console.WriteLine(string.Format("    {0}", b.Header.HashAsString));
                Console.WriteLine(string.Format("  New part of chain (top to bottom):"));
                foreach (var b in newBlocks) Console.Write(string.Format("    {0}", b.Header.HashAsString));
                ...
                Console.WriteLine(affectedUs ? "Re-org affected our transactions" : "Re-org had no effect on our transactions");
                ...
                foreach (var tx in onlyOldChainTransactions.Values) Console.WriteLine(string.Format("  Only Old: {0}", tx.HashAsString));
                foreach (var tx in oldChainTransactions.Values) Console.WriteLine(string.Format("  Old: {0}", tx.HashAsString));
                foreach (var tx in newChainTransactions.Values) Console.WriteLine(string.Format("  New: {0}", tx.HashAsString));
                ...
                foreach (var tx in commonChainTransactions.Values)
                {
                    ...
                    if (unspentOutputs > 0)
                    {
                        Console.WriteLine(string.Format("  TX {0}: ->unspent", tx.HashAsString));
                        Unspent[tx.Hash] = tx;
                    }
                    else
                    {
                        Console.WriteLine(string.Format("  TX {0}: ->spent", tx.HashAsString));
                        Spent[tx.Hash] = tx;
                    }
                }
                ...
                {
                    Console.WriteLine(string.Format("Replaying block {0}", b.Header.HashAsString));
                    ...
                    foreach (var tx in newChainTransactions.Values)
                    {
                        ...
                        {
                            txns.Add(tx);
                            Console.WriteLine(string.Format("  containing tx {0}", tx.HashAsString));
                        }
                    }
                ...
                Console.WriteLine("Reprocessing:");
                ...
                Console.WriteLine(string.Format("post-reorg balance is {0}", Utils.BitcoinValueToFriendlyString(GetBalance())));
        ...
        {
            Console.WriteLine(string.Format("  TX {0}", tx.HashAsString));
            ...
            foreach (var input in tx.Inputs)
            {
                ...
                else if (result == TransactionInput.ConnectionResult.AlreadySpent)
                {
                    ...
                    Console.WriteLine("   ->dead, will not confirm now unless there's another re-org");
