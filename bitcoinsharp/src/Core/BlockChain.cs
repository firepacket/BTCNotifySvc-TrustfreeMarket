...
namespace BitCoinSharp
{
    ...
    public class BlockChain
    {
        ...
        public BlockChain(NetworkParameters @params, IEnumerable<Wallet> wallets, IBlockStore blockStore)
        {
            ...
            Console.WriteLine(string.Format("chain head is:{0}{1}", Environment.NewLine, _chainHead.Header));
            _params = @params;
        ...
        private bool Add(Block block, bool tryConnecting)
        {
            lock (this)
            {
                if (Environment.TickCount - _statsLastTime > 1000)
                {
                    ...
                    Console.WriteLine(string.Format("{0} blocks per second", _statsBlocksAdded));
                ...
                {
                    Console.WriteLine("ERROR: " + "Failed to verify block:", e);
                    Console.WriteLine("ERROR: " + block.HashAsString);
                    throw;
                }
        ...
        private void ConnectBlock(StoredBlock newStoredBlock, StoredBlock storedPrev, IDictionary<Wallet, List<Transaction>> newTransactions)
        {
            ...
            else
            {
                ...
                if (haveNewBestChain)
                {
                    Console.WriteLine("Block is causing a re-organize");
                }
                else
                {
                    var splitPoint = FindSplit(newStoredBlock, _chainHead);
                    var splitPointHash = splitPoint != null ? splitPoint.Header.HashAsString : "?";
                    Console.WriteLine(string.Format("Block forks the chain at {0}, but it did not cause a reorganize:{1}{2}",
        ...
        private void HandleNewBestChain(StoredBlock newChainHead)
        {
            ...
            Console.WriteLine(string.Format("Re-organize after split at height {0}", splitPoint.Height));
            Console.WriteLine(string.Format("Old chain head: {0}", _chainHead.Header.HashAsString));
            Console.WriteLine(string.Format("New chain head: {0}", newChainHead.Header.HashAsString));
            Console.WriteLine(string.Format("Split at block: {0}", splitPoint.Header.HashAsString));
        ...
        private void TryConnectingUnconnected()
        {
            ...
            do
            {
                ...
                {
                    Console.WriteLine(string.Format("Connected {0} floating blocks.", blocksConnectedThisRound));
                }
