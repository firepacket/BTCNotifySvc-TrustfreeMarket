    ...
    public class BitcoinSerializer
    {
        ...
        public void Serialize(Message message, Stream @out)
        {
            ...
            if (_log.IsDebugEnabled)
                Console.WriteLine("Sending {0} message: {1}", name, Utils.BytesToHexString(header) + Utils.BytesToHexString(payload));
        }
        ...
        public Message Deserialize(Stream @in)
        {
            ...
            if (_log.IsDebugEnabled)
            {
                Console.WriteLine("Received {0} byte '{1}' message: {2}",
                                 size,
                                 command,
        ...
        private Message MakeMessage(string command, byte[] payloadBytes)
        {
            ...
            {
                return new UnknownMessage(_params, "alert", payloadBytes);
            }
