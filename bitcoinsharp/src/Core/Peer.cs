...
namespace BitCoinSharp
{
    ...
    public class Peer
    {
        ...
        public event Action<Transaction> OnTransactionEvent;
        public event Action<Message> OnUnknownMessageEvent;
        ...
        public void Run()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    else if (m is Transaction)
                    {
                        ...
                        OnTransactionEvent?.Invoke(m as Transaction);
                    }
                    ...
                    else
                    {
                        ...
                        OnUnknownMessageEvent?.Invoke(m);
                        Console.WriteLine(string.Format("Received unhandled message: {0}", m));
                    }
                }
            }
            catch (IOException e)
            {
                if (!_running)
                {
                    ...
                    Console.WriteLine("Shutting down peer loop");
                }
                else
                {
                    Disconnect();
            ...
            {
                Console.WriteLine("SCRIPT ERROR: " + se.Message);
            }
            catch (Exception e)
            {
                //Disconnect();
                Console.WriteLine("ERROR: " + e.Message);
                throw new Exception("ERROR: unexpected exception in peer loop", e);
            }
        ...
        private void ProcessBlock(Block m)
        {
            ...
            try
            {
                ...
                if (_blockChain.Add(m))
                {
                    ...
                    BlocksDownloaded?.Invoke(this, new BlocksDownloadedEventArgs(m, GetPeerBlocksToGet()));
                }
                else
                {
        ...
        private void ProcessInv(InventoryMessage inv)
        {
            ...
            foreach (var item in items)
            {
                ...
                //    Console.WriteLine("Got block inv");
                //if (item.Type == InventoryItem.ItemType.Transaction)
                //    Console.WriteLine("Got tx inv");
                //if (item.Type == InventoryItem.ItemType.Error)
                //    Console.WriteLine("got ERROR inv");
                if (item.Type == InventoryItem.ItemType.Transaction)
        ...
        private void BlockChainDownload(Sha256Hash toHash)
        {
            ...
            Console.WriteLine(string.Format("blockChainDownload({0})", toHash));
        ...
        public void StartBlockChainDownload()
        {
            ...
            {
                ChainDownloadStarted?.Invoke(this, new ChainDownloadStartedEventArgs(GetPeerBlocksToGet()));
